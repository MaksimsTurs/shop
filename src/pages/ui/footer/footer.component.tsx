...
import logo from './img/logo.png?format=webp&prest=thumbnail'

const Footer: FC = () => {
	const { categories } = useSelector<RootState, ProductInitialState>(
		state => state.products
	)

	const newCategoriesSlice: string[] = categories.slice(0, 8)
	...
	return (
		<footer className={style.footer_container}>
			<div className={style.footer_body}>
				...
				<div className={style.footer_social_media_container}>
					<svg
			...
			<div className={style.footer_body}>
				<h4>Find Product</h4>
				<ul>
					{newCategoriesSlice.map((element, index) => (
						<li key={index}>
							<Link to={`/search?${element.toLowerCase()}`}>{element}</Link>
						</li>
					))}
				</ul>
			</div>
